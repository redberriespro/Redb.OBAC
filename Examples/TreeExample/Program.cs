...
namespace TreeExample
{
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            await obacManager.SetTreeNodeAcl(treeId, catalog1.Id, new AclInfo
            {
                AclItems = new[]
                {
            ...
            var file1Perm  = await permChecker.GetObjectPermissions(treeId, file1.Id).ConfigureAwait(false);
        ...
        private static async Task<CatalogEntity> EnsureCatalog(TreeDbContext ctx, IObacObjectManager obacManager, Guid treeId, int id)
        {
            ...
            await obacManager.EnsureTreeNode(treeId, result.Entity.Id, null, 1, stringId:$"CATALOG:{result.Entity.Id}");
        ...
        private static async Task<FileEntity> EnsureFile(TreeDbContext ctx, IObacObjectManager obacManager, Guid treeId, int id, CatalogEntity? catalog = null)
        {
            ...
            // register object's replica in OBAC
            await obacManager.EnsureTreeNode(treeId, result.Entity.Id, catalog?.Id, 1, stringId:$"FILE:{result.Entity.Id}.{result.Entity.Version}");
